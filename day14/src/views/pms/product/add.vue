<template>
  <div class="addproduct">
    ...
    <el-card class="box-card">
      <span><i class="iconfont icon-dingdan1"></i> 填写商品信息</span>
  </el-card>
  <!-- 表单验证 -->
...
<script>
import {fetchCateList,fetchBrandList,fetchAddList} from '../../../api/product'
export default {
  name:'addpoduct',
  data:function(){
      return {
      ...
      submitForm:function(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            //调用后台接口
            fetchAddList(this.productValue).then(res=>{
              console.log(res);
              if(res.type == 'success'){
                this.$message({
                  message:'成功添加商品',
